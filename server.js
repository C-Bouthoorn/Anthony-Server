...
/*jshint node: true*///;
'use strict';
var Base64, CHANNELS_TABLE, FILES, HTML, PORT, SERVER_USER, USER_TABLE, WWW_ROOT, app, channels, child, cmdline, connectDatabase, createTexture, db, db_config, doCommands, encodeHTML, escapeRegex, fs, htmlencode, http, io, mysql, parseCommand, parseMessage, postlogin, qq, readline, receiveMessage, salthash, sendMessageAs, sendServerMessageTo, sessionid_by_socketid, sessions, sockets, stamp, util,
...
createTexture = function(type, color) {
  var out, p;
  if (!(type === "hair" || type === "tail")) {
...
connectDatabase = function() {
  ...
  db.connect(function(err) {
    if (err) {
      ...
      return console.log("[DATABASE]".c_OK + " Connected");
    }
  });
...
FILES = {
  ...
  recursive: ['/images', '/images/emoji', '/images/textures', '/images/textures/converted'],
  redir: {
    '/': '/chat.html',
...
db.query(qq, function(err, data) {
  ...
  for (i = 0, len = data.length; i < len; i++) {
    ...
    results.push(channels[chnl.name] = chnl.joined.split(';'));
  }
  return results;
});

http.listen(PORT, function() {
  return console.log("[  INFO  ]".c_OK + (" Server started on port " + PORT + "!"));
});
